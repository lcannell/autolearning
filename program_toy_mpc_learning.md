...
This is an experiment where the LLM improves a toy MPC benchmark by turning `toy_mpc_qp.py` into an autonomous search over the hyperparameters in `MPCParams`.
...
3. **Read the in-scope files**: The repo is small. Read these files for full context:
   ...
   - `toy_mpc_qp.py` — the file you modify. Hyperparameter calibration.
...
- Add code to find the best combination of the hyperparameters in `MPCParams`.
- You only need to test parameter values, evaluate `objective` performance, and infer the next hyperparameter combination to explore. Autonomously balance exploration and exploitation by learning how each parameter change affects outcomes.
...
**The goal is simple: get the lowest objective**
...
2. Tune `toy_mpc_qp.py` with an experimental idea
3. git commit
...
The idea is that you operate as a fully autonomous black-box researcher: you test configurations, observe only the outputs, and infer how to calibrate the hyperparameter “knobs” through iterative trial and error. Your goal is to continuously advance the branch through informed experimentation, learning the system purely from input-output behavior.
